 // components/Reports/SupplierBalanceReport.tsx
"use client";
...
interface SupplierBalanceReportItem {
  id: number;
  customerName: string;
  address: string;
  productName: string;
...
// --- START: Move ViewModeDropdown outside SupplierBalanceReport ---
...
// --- END: Moved ViewModeDropdown outside SupplierBalanceReport ---
...
const SupplierBalanceReport: React.FC<SupplierBalanceReportProps> = ({
  ...
  const [supplierBalanceReport, setSupplierBalanceReports] = useState<
    SupplierBalanceReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: SupplierBalanceReportItem[] = await res.json();
      setSupplierBalanceReports(data); // Store the fetched data in state
      ...
      console.error("Error fetching SupplierBalanceReport:", err);
      setError("Failed to fetch SupplierBalanceReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the supplierBalanceReport array is empty
    if (supplierBalanceReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [supplierBalanceReport]); // Dependency array includes supplierBalanceReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? supplierBalanceReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setSupplierBalanceReports([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      supplierBalanceReport.length > 0 &&
        selectedIds.length === supplierBalanceReport.length
    );
  }, [selectedIds, supplierBalanceReport]); // Depend on selectedIds and supplierBalanceReport
  ...
  // Check if supplierBalanceReport is empty after loading and filtering
  if (supplierBalanceReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(supplierBalanceReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {supplierBalanceReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{supplierBalanceReport.length}</span> of{" "}
          <span className="text-blue-600">{supplierBalanceReport.length}</span>
        </span>
      </footer>
