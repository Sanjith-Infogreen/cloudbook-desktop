 // components/Reports/B2BReport.tsx
"use client";
...
// --- START: Move ViewModeDropdown outside B2BReport ---
...
// --- END: Moved ViewModeDropdown outside B2BReport ---


const B2BReport: React.FC<B2BReportProps> = ({
  activeReport,
}) => {
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: B2BReportItem[] = await res.json();
      setB2BReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching B2BReport:", err);
      setError("Failed to fetch B2BReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the b2bReport array is empty
    if (b2bReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [b2bReport]); // Dependency array includes b2bReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? b2bReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setB2BReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      b2bReport.length > 0 &&
        selectedIds.length === b2bReport.length
    );
  }, [selectedIds, b2bReport]); // Depend on selectedIds and b2bReport
  ...
  // Check if b2bReport is empty after loading and filtering
  if (b2bReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(b2bReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {b2bReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{b2bReport.length}</span> of{" "}
          <span className="text-blue-600">{b2bReport.length}</span>
        </span>
      </footer>
