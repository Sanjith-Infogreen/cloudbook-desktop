 // components/Reports/CashbookReport.tsx
"use client";
...
// --- START: Move ViewModeDropdown outside CashbookReport ---
...
// --- END: Moved ViewModeDropdown outside CashbookReport ---


const CashbookReport: React.FC<CashbookReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [cashbookReport, setCashbookReport] = useState<
    CashbookReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: CashbookReportItem[] = await res.json();
      setCashbookReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching CashbookReport:", err);
      setError("Failed to fetch CashbookReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the cashbookReport array is empty
    if (cashbookReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [cashbookReport]); // Dependency array includes cashbookReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? cashbookReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setCashbookReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      cashbookReport.length > 0 &&
        selectedIds.length === cashbookReport.length
    );
  }, [selectedIds, cashbookReport]); // Depend on selectedIds and cashbookReport
  ...
  // Check if cashbookReport is empty after loading and filtering
  if (cashbookReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(cashbookReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-148px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {cashbookReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{cashbookReport.length}</span> of{" "}
          <span className="text-blue-600">{cashbookReport.length}</span>
        </span>
      </footer>
