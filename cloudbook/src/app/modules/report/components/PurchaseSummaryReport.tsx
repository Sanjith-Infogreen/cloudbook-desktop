 // components/Reports/PurchaseSummaryReport.tsx
"use client";
...
interface PurchaseSummaryReportItem {
  id: number;
  customerName: string;
  address: string;
  productName: string;
...
// --- START: Move ViewModeDropdown outside PurchaseSummaryReport ---
...
// --- END: Moved ViewModeDropdown outside PurchaseSummaryReport ---


const PurchaseSummaryReport: React.FC<PurchaseSummaryReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [purchaseSummaryReport, setPurchaseSummaryReport] = useState<
    PurchaseSummaryReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: PurchaseSummaryReportItem[] = await res.json();
      setPurchaseSummaryReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching PurchaseSummaryReport:", err);
      setError("Failed to fetch PurchaseSummaryReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the purchaseSummaryReport array is empty
    if (purchaseSummaryReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [purchaseSummaryReport]); // Dependency array includes purchaseSummaryReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? purchaseSummaryReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setPurchaseSummaryReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      purchaseSummaryReport.length > 0 &&
        selectedIds.length === purchaseSummaryReport.length
    );
  }, [selectedIds, purchaseSummaryReport]); // Depend on selectedIds and purchaseSummaryReport
  ...
  // Check if purchaseSummaryReport is empty after loading and filtering
  if (purchaseSummaryReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(purchaseSummaryReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-148px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {purchaseSummaryReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{purchaseSummaryReport.length}</span> of{" "}
          <span className="text-blue-600">{purchaseSummaryReport.length}</span>
        </span>
      </footer>
