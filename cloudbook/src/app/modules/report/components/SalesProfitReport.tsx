 // components/Reports/SalesProfitReport.tsx
"use client";
...
interface SalesProfitReportItem {
  id: number;
  customerName: string;
  address: string;
  productName: string;
...
// --- START: Move ViewModeDropdown outside SalesProfitReport ---
...
// --- END: Moved ViewModeDropdown outside SalesProfitReport ---


const SalesProfitReport: React.FC<SalesProfitReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [salesProfitReport, setSalesProfitReports] = useState<
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: SalesProfitReportItem[] = await res.json();
      setSalesProfitReports(data); // Store the fetched data in state
      ...
      console.error("Error fetching SalesProfitReport:", err);
      setError("Failed to fetch SalesProfitReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the salesProfitReport array is empty
    if (salesProfitReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [salesProfitReport]); // Dependency array includes salesProfitReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? salesProfitReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setSalesProfitReports([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      salesProfitReport.length > 0 &&
        selectedIds.length === salesProfitReport.length
    );
  }, [selectedIds, salesProfitReport]); // Depend on selectedIds and salesProfitReport
  ...
  // Check if salesProfitReport is empty after loading and filtering
  if (salesProfitReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(salesProfitReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-148px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {salesProfitReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{salesProfitReport.length}</span> of{" "}
          <span className="text-blue-600">{salesProfitReport.length}</span>
        </span>
      </footer>
