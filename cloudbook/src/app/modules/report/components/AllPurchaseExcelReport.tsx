 // components/Reports/AllPurchaseExcelReport.tsx
"use client";
...
// --- START: Move ViewModeDropdown outside AllPurchaseExcelReport ---
...
// --- END: Moved ViewModeDropdown outside AllPurchaseExcelReport ---


const AllPurchaseExcelReport: React.FC<AllPurchaseExcelReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [allPurchaseExcelReport, setAllPurchaseExcelReport] = useState<
    AllPurchaseExcelReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: AllPurchaseExcelReportItem[] = await res.json();
      setAllPurchaseExcelReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching AllPurchaseExcelReport:", err);
      setError("Failed to fetch AllPurchaseExcelReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the allPurchaseExcelReport array is empty
    if (allPurchaseExcelReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [allPurchaseExcelReport]); // Dependency array includes allPurchaseExcelReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? allPurchaseExcelReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setAllPurchaseExcelReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      allPurchaseExcelReport.length > 0 &&
        selectedIds.length === allPurchaseExcelReport.length
    );
  }, [selectedIds, allPurchaseExcelReport]); // Depend on selectedIds and allPurchaseExcelReport
  ...
  // Check if allPurchaseExcelReport is empty after loading and filtering
  if (allPurchaseExcelReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(allPurchaseExcelReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {allPurchaseExcelReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{allPurchaseExcelReport.length}</span> of{" "}
          <span className="text-blue-600">{allPurchaseExcelReport.length}</span>
        </span>
      </footer>
...
export default AllPurchaseExcelReport;