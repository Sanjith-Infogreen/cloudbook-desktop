 // components/Reports/StatementReport.tsx
"use client";
...
// --- START: Move ViewModeDropdown outside StatementReport ---
...
// --- END: Moved ViewModeDropdown outside StatementReport ---
...
const StatementReport: React.FC<StatementReportProps> = ({
  ...
  const [statementReport, setStatementReports] = useState<
    StatementReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: StatementReportItem[] = await res.json();
      setStatementReports(data); // Store the fetched data in state
      ...
      console.error("Error fetching StatementReport:", err);
      setError("Failed to fetch StatementReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the statementReport array is empty
    if (statementReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [statementReport]); // Dependency array includes statementReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? statementReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setStatementReports([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      statementReport.length > 0 &&
        selectedIds.length === statementReport.length
    );
  }, [selectedIds, statementReport]); // Depend on selectedIds and statementReport
  ...
  // Check if statementReport is empty after loading and filtering
  if (statementReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(statementReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-148px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {statementReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{statementReport.length}</span> of{" "}
          <span className="text-blue-600">{statementReport.length}</span>
        </span>
      </footer>
