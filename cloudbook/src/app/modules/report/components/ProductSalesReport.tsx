 // components/Reports/ProductSalesReport.tsx
"use client";
...
// Define the ProductSalesReportProps interface as per AllProductSalesReportList
...
interface ProductSaleReportItem {
  id: number;
  customerName: string;
  address: string;
  productName: string;
...
// --- START: Move ViewModeDropdown outside ProductSalesReport ---
...
// --- END: Moved ViewModeDropdown outside ProductSalesReport ---


const ProductSalesReport: React.FC<ProductSalesReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [productSalesReports, setProductSalesReports] = useState<
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: ProductSaleReportItem[] = await res.json();
      setProductSalesReports(data); // Store the fetched data in state
      ...
      console.error("Error fetching ProductSalesReport:", err);
      setError("Failed to fetch ProductSalesReport: " + err.message);
    } finally {
...
useEffect(() => {
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedIds(checked ? productSalesReports.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setProductSalesReports([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      productSalesReports.length > 0 &&
        selectedIds.length === productSalesReports.length
    );
  }, [selectedIds, productSalesReports]); // Depend on selectedIds and productSalesReports
  ...
  // Check if productSalesReports is empty after loading and filtering
  if (productSalesReports.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(productSalesReports.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-148px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {productSalesReports.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{productSalesReports.length}</span> of{" "}
          <span className="text-blue-600">{productSalesReports.length}</span>
        </span>
      </footer>
