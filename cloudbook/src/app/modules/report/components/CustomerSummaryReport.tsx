...
interface CustomerSummaryReportItem {
  id: number;
  customerName: string;
  address: string;
  productName: string;
...
// --- START: Move ViewModeDropdown outside CustomerSummaryReport ---
...
// --- END: Moved ViewModeDropdown outside CustomerSummaryReport ---
...
const CustomerSummaryReport: React.FC<CustomerSummaryReportProps> = ({
  ...
  const [customerSummaryReport, setCustomerSummaryReport] = useState<
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: CustomerSummaryReportItem[] = await res.json();
      setCustomerSummaryReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching CustomerSummaryReport:", err);
      setError("Failed to fetch CustomerSummaryReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the customerSummaryReport array is empty
    if (customerSummaryReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [customerSummaryReport]); // Dependency array includes customerSummaryReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? customerSummaryReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setCustomerSummaryReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      customerSummaryReport.length > 0 &&
        selectedIds.length === customerSummaryReport.length
    );
  }, [selectedIds, customerSummaryReport]); // Depend on selectedIds and customerSummaryReport
  ...
  // Check if customerSummaryReport is empty after loading and filtering
  if (customerSummaryReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(customerSummaryReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {customerSummaryReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{customerSummaryReport.length}</span> of{" "}
          <span className="text-blue-600">{customerSummaryReport.length}</span>
        </span>
      </footer>
