 // components/Reports/SalesSummaryReport.tsx
"use client";
...
interface SalesSummaryReportItem {
  id: number;
  customerName: string;
  address: string;
  productName: string;
...
// --- START: Move ViewModeDropdown outside SalesSummaryReport ---
...
// --- END: Moved ViewModeDropdown outside SalesSummaryReport ---
...
const SalesSummaryReport: React.FC<SalesSummaryReportProps> = ({
  ...
  const [salesSummaryReport, setSalesSummaryReport] = useState<
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: SalesSummaryReportItem[] = await res.json();
      setSalesSummaryReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching SalesSummaryReport:", err);
      setError("Failed to fetch SalesSummaryReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the salesSummaryReport array is empty
    if (salesSummaryReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [salesSummaryReport]); // Dependency array includes salesSummaryReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? salesSummaryReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setSalesSummaryReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      salesSummaryReport.length > 0 &&
        selectedIds.length === salesSummaryReport.length
    );
  }, [selectedIds, salesSummaryReport]); // Depend on selectedIds and salesSummaryReport
  ...
  // Check if salesSummaryReport is empty after loading and filtering
  if (salesSummaryReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(salesSummaryReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {salesSummaryReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{salesSummaryReport.length}</span> of{" "}
          <span className="text-blue-600">{salesSummaryReport.length}</span>
        </span>
      </footer>
