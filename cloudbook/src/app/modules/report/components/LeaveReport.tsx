...
import { setLeaveReports } from "@/store/report/leaveReport";

// Define the LeaveReport interface as per AllLeaveReportList
...
const LeaveReport: React.FC<LeaveReportProps> = ({
  ...
  const leaveReport = useSelector(
    (state: RootState) => state.leaveReport.leaveReport
  );
  const fetchLeaveReport = async () => {
    try {
      ...
      const res = await fetch("http://localhost:4000/mockLeaveReportData");
      ...
      dispatch(setLeaveReports(data));
      // Simulate API delay

      // Apply filters to mockData
    } catch (err) {
      console.error("Error fetching LeaveReport:", err);
      setError("Failed to fetch LeaveReport");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (leaveReport.length === 0) fetchLeaveReport();
  }, []); // Refetch when filters change

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    setSelectedIds(checked ? leaveReport.map((t) => t.id) : []);
  };
  ...
  useEffect(() => {
    setSelectAll(
      leaveReport.length > 0 && selectedIds.length === leaveReport.length
    );
  }, [selectedIds, leaveReport]);
  ...
  if (leaveReport.length === 0 && Object.keys(filters).length === 0) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg text-gray-500">
          No LeaveReport data available.
        </div>
      </div>
    );
  } else if (leaveReport.length === 0 && Object.keys(filters).length > 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        <div className="text-lg text-gray-500">
          No LeaveReport found matching your filters.
        </div>
        <button
  ...
  return (
    <main className="flex-1">
      <h1 className="text-[18px] p-2 sm:text-[20px] font-medium text-[#009333]">
        {activeReport}
      </h1>
      ...
      <div className="flex justify-between items-center px-1.5 py-1.5 bg-[#ebeff3]">
          ...
          {!selectedIds.length && (
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  setSelectedIds(leaveReport.map((t) => t.id));
                }}
              >
        ...
        <div className="flex items-center relative space-x-2">
          <input
            ...
            onChange={(e) =>
              handleLocalFilterChange("fromDate", e.target.value)
            }
          />
          <button
            className="btn-sm !border-transparent !text-[#384551] hover:bg-[#dce0e5] hover:border-[#ebeff3] text-sm"
      ...
      <div className="bg-[#ebeff3]">
        {selectedIds.length > 1 && (
          <div className="fixed top-42 left-1/2 transform -translate-x-1/2 z-50 badge-selected">
            {selectedIds.length} LeaveReports selected
          </div>
        )}
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {leaveReport.map((leaveReport, index) => (
                  <tr
                    key={leaveReport.id}
                    className={`group hover:bg-[#f5f7f9] text-sm cursor-pointer ${
                      selectedIds.includes(leaveReport.id)
                  ...
                  >
                    <td className="td-cell">
                      <input
                        type="checkbox"
                        className="form-check"
                        checked={selectedIds.includes(leaveReport.id)}
                        onChange={() => handleCheckboxChange(leaveReport.id)}
                      />
                    </td>
                    <td className="td-cell">
                    ...
                    <td className="td-cell">{leaveReport.employeeName}</td>
                    <td className="td-cell">{leaveReport.leaveType}</td>
                    <td className="td-cell">{leaveReport.fromDate}</td>
                    <td className="td-cell">{leaveReport.toDate}</td>
                    <td className="td-cell">{leaveReport.reason}</td>
                    <td className="td-cell">{leaveReport.status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing <span className="text-red-600">{leaveReport.length}</span> of{" "}
          <span className="text-blue-600">{leaveReport.length}</span>
        </span>
      </footer>
