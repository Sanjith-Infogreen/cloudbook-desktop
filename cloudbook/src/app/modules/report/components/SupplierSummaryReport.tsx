 // components/Reports/SupplierSummaryReport.tsx
"use client";
...
interface SupplierSummaryReportItem {
  id: number;
  customerName: string;
  address: string;
  productName: string;
...
// --- START: Move ViewModeDropdown outside SupplierSummaryReport ---
...
// --- END: Moved ViewModeDropdown outside SupplierSummaryReport ---


const SupplierSummaryReport: React.FC<SupplierSummaryReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [supplierSummaryReport, setSupplierSummaryReport] = useState<
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: SupplierSummaryReportItem[] = await res.json();
      setSupplierSummaryReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching SupplierSummaryReport:", err);
      setError("Failed to fetch SupplierSummaryReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the supplierSummaryReport array is empty
    if (supplierSummaryReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [supplierSummaryReport]); // Dependency array includes supplierSummaryReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? supplierSummaryReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setSupplierSummaryReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      supplierSummaryReport.length > 0 &&
        selectedIds.length === supplierSummaryReport.length
    );
  }, [selectedIds, supplierSummaryReport]); // Depend on selectedIds and supplierSummaryReport
  ...
  // Check if supplierSummaryReport is empty after loading and filtering
  if (supplierSummaryReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(supplierSummaryReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-148px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {supplierSummaryReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{supplierSummaryReport.length}</span> of{" "}
          <span className="text-blue-600">{supplierSummaryReport.length}</span>
        </span>
      </footer>
