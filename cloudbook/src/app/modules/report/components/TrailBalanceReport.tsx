 // components/Reports/TrailBalanceReport.tsx
"use client";
...
// --- START: Move ViewModeDropdown outside TrailBalanceReport ---
...
// --- END: Moved ViewModeDropdown outside TrailBalanceReport ---


const TrailBalanceReport: React.FC<TrailBalanceReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [trailBalanceReport, setTrailBalanceReport] = useState<
    TrailBalanceReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: TrailBalanceReportItem[] = await res.json();
      setTrailBalanceReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching TrailBalanceReport:", err);
      setError("Failed to fetch TrailBalanceReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the trailBalanceReport array is empty
    if (trailBalanceReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [trailBalanceReport]); // Dependency array includes trailBalanceReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? trailBalanceReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setTrailBalanceReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      trailBalanceReport.length > 0 &&
        selectedIds.length === trailBalanceReport.length
    );
  }, [selectedIds, trailBalanceReport]); // Depend on selectedIds and trailBalanceReport
  ...
  // Check if trailBalanceReport is empty after loading and filtering
  if (trailBalanceReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(trailBalanceReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {trailBalanceReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{trailBalanceReport.length}</span> of{" "}
          <span className="text-blue-600">{trailBalanceReport.length}</span>
        </span>
      </footer>
