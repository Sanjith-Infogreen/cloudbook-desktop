 // components/Reports/DaybookReport.tsx
"use client";
...
// --- START: Move ViewModeDropdown outside DaybookReport ---
...
// --- END: Moved ViewModeDropdown outside DaybookReport ---


const DaybookReport: React.FC<DaybookReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [daybookReport, setDaybookReport] = useState<
    DaybookReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: DaybookReportItem[] = await res.json();
      setDaybookReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching DaybookReport:", err);
      setError("Failed to fetch DaybookReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the daybookReport array is empty
    if (daybookReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [daybookReport]); // Dependency array includes daybookReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? daybookReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setDaybookReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      daybookReport.length > 0 &&
        selectedIds.length === daybookReport.length
    );
  }, [selectedIds, daybookReport]); // Depend on selectedIds and daybookReport
  ...
  // Check if daybookReport is empty after loading and filtering
  if (daybookReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(daybookReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {daybookReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{daybookReport.length}</span> of{" "}
          <span className="text-blue-600">{daybookReport.length}</span>
        </span>
      </footer>
