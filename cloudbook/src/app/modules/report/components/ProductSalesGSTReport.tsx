 // components/Reports/ProductSalesGSTReport.tsx
"use client";
...
// --- START: Move ViewModeDropdown outside ProductSalesGSTReport ---
...
// --- END: Moved ViewModeDropdown outside ProductSalesGSTReport ---


const ProductSalesGSTReport: React.FC<ProductSalesGSTReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [productSalesGSTReport, setProductSalesGSTReport] = useState<
    ProductSalesGSTReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: ProductSalesGSTReportItem[] = await res.json();
      setProductSalesGSTReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching ProductSalesGSTReport:", err);
      setError("Failed to fetch ProductSalesGSTReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the productSalesGSTReport array is empty
    if (productSalesGSTReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [productSalesGSTReport]); // Dependency array includes productSalesGSTReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? productSalesGSTReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setProductSalesGSTReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      productSalesGSTReport.length > 0 &&
        selectedIds.length === productSalesGSTReport.length
    );
  }, [selectedIds, productSalesGSTReport]); // Depend on selectedIds and productSalesGSTReport
  ...
  // Check if productSalesGSTReport is empty after loading and filtering
  if (productSalesGSTReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(productSalesGSTReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-187px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {productSalesGSTReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{productSalesGSTReport.length}</span> of{" "}
          <span className="text-blue-600">{productSalesGSTReport.length}</span>
        </span>
      </footer>
...
export default ProductSalesGSTReport;