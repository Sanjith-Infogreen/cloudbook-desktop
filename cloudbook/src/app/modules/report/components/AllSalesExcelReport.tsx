 // components/Reports/AllSalesExcelReport.tsx
"use client";
...
// --- START: Move ViewModeDropdown outside AllSalesExcelReport ---
...
// --- END: Moved ViewModeDropdown outside AllSalesExcelReport ---


const AllSalesExcelReport: React.FC<AllSalesExcelReportProps> = ({
  activeReport,
}) => {
  // State to hold the fetched product sales report data
  const [allSalesExcelReport, setAllSalesExcelReport] = useState<
    AllSalesExcelReportItem[]
  >([]);
  ...
  const fetchProductSalesReport = async () => {
    try {
      ...
      const data: AllSalesExcelReportItem[] = await res.json();
      setAllSalesExcelReport(data); // Store the fetched data in state
      ...
      console.error("Error fetching AllSalesExcelReport:", err);
      setError("Failed to fetch AllSalesExcelReport: " + err.message);
    } finally {
  ...
  useEffect(() => {
    // Only fetch if the allSalesExcelReport array is empty
    if (allSalesExcelReport.length === 0) {
      fetchProductSalesReport();
    }
  }, [allSalesExcelReport]); // Dependency array includes allSalesExcelReport to prevent infinite loops if data changes

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setSelectAll(checked);
    // Map over the actual data array, not the component name
    setSelectedIds(checked ? allSalesExcelReport.map((t) => t.id) : []);
  };
  ...
  const handleRefresh = () => {
    ...
    setAllSalesExcelReport([]); // Clear current data to force refetch
  ...
  useEffect(() => {
    ...
    setSelectAll(
      allSalesExcelReport.length > 0 &&
        selectedIds.length === allSalesExcelReport.length
    );
  }, [selectedIds, allSalesExcelReport]); // Depend on selectedIds and allSalesExcelReport
  ...
  // Check if allSalesExcelReport is empty after loading and filtering
  if (allSalesExcelReport.length === 0 && Object.keys(filters).length === 0) {
    return (
    ...
    <main className="flex-1">
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  setSelectAll(true);
                  // Map over the actual data array, not the component name
                  setSelectedIds(allSalesExcelReport.map((t) => t.id));
                }}
              >
        ...
        <div className="mx-2 h-[calc(100vh-148px)] overflow-hidden rounded-lg bg-white">
          <div className="h-full overflow-y-auto">
            <table className="w-full">
              ...
              <tbody>
                {/* Map over the actual data array, not the component name */}
                {allSalesExcelReport.map((reportItem, index) => (
                  <tr
                    key={reportItem.id}
      ...
      <footer className="bg-[#ebeff3] py-3 h-[56.9px] px-4 flex items-center justify-start">
        <span className="text-sm">
          Showing{" "}
          <span className="text-red-600">{allSalesExcelReport.length}</span> of{" "}
          <span className="text-blue-600">{allSalesExcelReport.length}</span>
        </span>
      </footer>
...
export default AllSalesExcelReport;